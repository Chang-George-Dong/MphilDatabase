...
def save_test_duration(file_name, test_duration):
    res = pd.DataFrame(
        data=np.zeros((1, 1), dtype=float), index=[0], columns=["test_duration"]
    )
...
def normalize_norm(v, eps=None, avoid_zero_div=1e-12):
    ...
    square = tf.maximum(
        avoid_zero_div, tf.reduce_sum(tf.square(v), axis=axis, keepdims=True)
    )
    if eps is None:
...
def prepare_data(datasets_dict, dataset_name):
    ...
    y_true_train = y_train.astype(int)
...
def read_all_datasets(root_dir, archive_name):
    ...
    if archive_name == "UCRArchive_2018":
        ...
        for dataset_name in DATASET_NAMES:
            ...
            try:
                df_train = pd.read_csv(
                    os.path.join(root_dir_dataset, dataset_name + "_TRAIN.tsv"),
                    sep="\t",
                    header=None,
                    encoding="latin-1",
                )
                df_test = pd.read_csv(
                    os.path.join(root_dir_dataset, dataset_name + "_TEST.tsv"),
                    sep="\t",
                    header=None,
                    encoding="latin-1",
                )
                ...
                print(os.path.join(root_dir_dataset, dataset_name + "_TRAIN.tsv"))
                continue
        ...
        for dataset_name in DATASET_NAMES:
            root_dir_dataset = os.path.join(
                root_dir, "archives", archive_name, dataset_name
            )
...
def calculate_smoothness(series):
    return np.mean(np.abs(series[1:] - series[:-1]))
...
def calculate_metrics(y_true, y_pred, duration, y_true_val=None, y_pred_val=None):
    ...
    res["smoothness"] = calculate_smoothness(y_pred)
    return res
...
def get_attack_from_name(attack_name):
    ...
    if attack_name == "SGM":
        ...
        return SGM
    if attack_name == "GM_l2":
        from attacks.gm_l2 import GM_l2

        return GM_l2
    if attack_name == "GM":
        from attacks.gm import GM

        return GM
    if attack_name == "KL_l2":
        from attacks.kL_l2 import KL_l2

        return KL_l2
    if attack_name == "KL":
        from attacks.kl import KL

        return KL
    if attack_name == "KL_l2l1":
        from attacks.KL_l2l1 import KL_l2l1

        return KL_l2l1
    if attack_name == "GM_ratio_T":
        from attacks.gm_ratio_T import GM_ratio
...
def read_results_csv_attack(root_dir_results, metric="asr"):
    ...
    res = pd.DataFrame(
        data=np.zeros((0, len(columns)), dtype=float), index=[], columns=columns
    )
    ...
    res = pd.DataFrame(
        {
            metric: res.groupby(["attack_name", "archive_name", "dataset_name"])[
                metric
            ].mean()
        }
    ).reset_index()
...
def plot_pairwise(
    root_dir, method_1, method_2, metrics=["asr", "avg_distance", "smoothness"]
):
    print("Pairwise %s vs %s" % (method_1, method_2))
    print("- metrics: {}\n".format(metrics))

    n_metrics = len(metrics)
    fig, axs = plt.subplots(1, n_metrics, figsize=(7 * n_metrics, 4))

    for i_metric in range(n_metrics):
        res_df, not_completed = read_results_csv_attack(
            root_dir, metric=metrics[i_metric]
        )

        sorted_df = res_df.loc[
            (res_df["attack_name"] == method_1) | (res_df["attack_name"] == method_2)
        ...
        for not_completed_dname in not_completed:
            sorted_df.drop(
                sorted_df.loc[sorted_df["dataset_name"] == not_completed_dname].index,
                axis=0,
                inplace=True,
            )
        sorted_df.reset_index(inplace=True)
        ...
        data = (
            np.array(sorted_df[metrics[i_metric]])
            .reshape(m, max_nb_datasets)
            .transpose()
        )

        # concat the dataset name and the archive name to put them in the columns s
        sorted_df["archive_dataset_name"] = (
            sorted_df["archive_name"] + "__" + sorted_df["dataset_name"]
        )
        ...
        axs[i_metric].set_xlabel(
            "{}\n({} {}, draws {}, {} {})".format(
                method_1,
                get_short_method_name(method_1),
                wins,
                draws,
                get_short_method_name(method_2),
                losses,
            ),
            fontsize="large",
        )
    ...
    name_plot = method_1 + "-vs-" + method_2 + "-multi-metrics-pairwise.pdf"
...
def plot_compare_perturbed_series(root_dir, method_1, method_2, iteration):
    ...
    for dataset_name in dnames:
        ...
        axs[i, j].plot(
            p_x_test_1[idx] - x_test[idx],
            color="blue",
            label=method_1,
            linewidth=linewidth,
        )
        axs[i, j].plot(
            p_x_test_2[idx] - x_test[idx],
            color="red",
            label=method_2,
            linewidth=linewidth,
        )
...
def save_k_dist(k, nb_class, cm_class, origin_dist, perturb_dist, output_dir):
    df_dist = pd.DataFrame(
        columns=[
            ...
            "class_perturb",
            "n_samples",
            "avg_dist_class_origin",
            "avg_dist_class_perturb",
        ]
    )
