...
class GM(Attack):
    def __init__(
        self, model_dir, x_test, y_test, out_dir, eps_init=0.001, nb_iter=1000, eps=0.1
    ):
        super().__init__()
    ...
    def frequency_attenuation(self, r, mode="low", alpha=1.0):
        """
        ...
        # Compute FFT of r
        R = np.fft.fft(r)
        frequencies = np.fft.fftfreq(r.shape[1])
        frequencies = frequencies[np.newaxis, :, np.newaxis]

        # Create attenuation curve
        if mode == "low":
            curve = np.exp(-alpha * np.abs(frequencies))
        elif mode == "high":
            curve = 1.0 - np.exp(-alpha * np.abs(frequencies))

        # Apply attenuation curve
        R *= curve

        # Compute IFFT to get modified perturbation
        modified_r = np.real(np.fft.ifft(R))
    ...
    def perturb_demo(self):
        ...
        for i in range(self.batch_size, n + self.batch_size, self.batch_size):
            ...
            r_data = self.frequency_attenuation(r_data, mode="high", alpha=5)
