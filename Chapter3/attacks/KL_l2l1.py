...
class KL_l2l1(Attack):
    def __init__(self, model_dir, x_test, y_test, out_dir,
                 eps_init=0.001, nb_iter=1000, smoothness=0.01, mu=1.0, eps=0.1,
                 alpha = 0.01,lamb = 1, gamma = 0.48 , beta = 0.01):
        super().__init__()
    ...
    def _loss_function(self, x, r, y_target):
        ...
        entropy = tf.nn.softmax_cross_entropy_with_logits(labels=y_target, logits=self.f(x_r))
        entropy_reg = self.beta * entropy  # beta is a hyperparameter

        return tf.reduce_mean(kl_loss + l2_loss + fused_lasso + entropy_reg)  # Add entropy regularization
