...
class GM_ratio(Attack):
    ...
    def _get_y_target(self, x, method='average_case'):
        ...
        if method == 'average_case':
            ...
            for i in range(len(y_pred)):
                ...
                random_indices.append(new_c)

            random_idx = np.array(random_indices)

            target_p_logits = tf.constant(target_p_logits, dtype=tf.float32)
            return tf.nn.softmax(target_p_logits), y_pred, random_idx
        else:
            raise Exception('Chosen method not defined')

    # def select_p_random(self, y_pred):
  
    #     random_indices = np.random.choice(y_pred.shape[1], size=y_pred.shape[0])

    #     # Use these indices to select one probability for each sample
    #     p_random = y_pred[np.arange(y_pred.shape[0]), random_indices]

    #     return p_random


    def _loss_function(self, x, r, y_target,random_idx):
        x_r = x + r
        py_pred = self.f(x_r)
        
        p_random = py_pred.numpy()[np.arange(py_pred.shape[0]), random_idx]
        pmax = tf.math.reduce_max(py_pred,axis = 1)

        ratio = (p_random+self.epsilon)/(pmax +self.epsilon)
    ...
    def perturb(self):
        ...
        for i in range(self.batch_size, n + self.batch_size, self.batch_size):
            ...
            y_target, y_pred,random_idx= self._get_y_target(x)

            loss_func = lambda: self._loss_function(x, r, y_target,random_idx)

            for itr in range(self.nb_iter):
                opt.minimize(loss_func, var_list=[r])
                losses[itr] = losses[itr] + self._loss_function(x, r, y_target,random_idx)
