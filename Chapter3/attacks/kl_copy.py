    ...
    def _get_y_target(self, x, method='average_case'):
        ...
        if method == 'average_case':
            ...
            for i in range(len(y_pred)):
                # 获取前两个最高的类别标签
                c_top2 = np.argsort(y_pred[i])[-self.top:][::-1]

                # 获取最高和次高类别的logits
                logits_top1, logits_top2 = target_p_logits[i, c_top2[0]], target_p_logits[i, c_top2[self.top-1]]

                # 计算这两个logits的平均值，并分别添加和减去0.01
                sum_ = logits_top1 + logits_top2
                target_p_logits[i, c_top2[0]], target_p_logits[i, c_top2[self.top-1]] = sum_ * self.gamma, sum_ * (1- self.gamma)
