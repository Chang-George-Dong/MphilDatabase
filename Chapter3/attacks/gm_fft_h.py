...
class GM_fft(Attack):
    ...
    def frequency_integration_fft(self, input_array):
        # Compute the FFT of the input array and obtain the magnitude (amplitude)
        fft_magnitude = tf.abs(tf.signal.rfft(input_array))

        # Compute the frequency values corresponding to the FFT
        n = input_array.shape[0]
        freq_values_np = np.arange(0, n // 2 + 1) / n * 2
        freq_values = tf.convert_to_tensor(freq_values_np, dtype=tf.float32)

        # # Parameters for the custom weight function
        # n_power = 10  # Adjust the power of the exponent based on your needs

        # # Calculate the weights using the power function
        # weights = (1 - freq_values) ** n_power

        # Parameters for the custom weight function
        c = 0.8  # center of the function, you can adjust this value
        k = 20  # steepness of the transition, adjust based on your needs

        # Calculate the weights using the custom function
        # weights = 1 / (1 + tf.exp(k * (freq_values - c)))

        # Parameters for the custom weight function
        # c = 0.5  # center of the function, adjust based on your needs
        # k = 20  # controls the width of the Gaussian, adjust based on your needs

        # Calculate the weights using the Gaussian function
        weights = tf.exp(-k * tf.square(freq_values - c))

        weighted_fft = fft_magnitude * weights

        # Integrate the weighted FFT result using TensorFlow's trapezoidal rule
        integrated_result = tf.reduce_sum(weighted_fft) / n**2
        if self.debug:
            print(
                "fft_magnitude:",
                fft_magnitude,
    ...
    def _loss_function(self, x, r, cur_x_test, y_target):
        ...
        # ... you can reintroduce the commented code related to cosine similarity if needed ...

        fftloss = self.frequency_integration_fft(r)
        ...
        return tf.reduce_mean(
            ce_loss + fftloss / 10 + l2_loss
        )  # Note: Removed self.b / self.b
    ...
    def draw(self):
        ...
        def calculate_js(y_before, y_after):
            ...
            # 计算JSD
            return 0.5 * entropy(y_before, M, axis=1) + 0.5 * entropy(
                y_after, M, axis=1
            )
