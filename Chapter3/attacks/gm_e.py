...
class GM(Attack):
    ...
    def _loss_function(self, x, r, y_target):
        ...
        entropy_loss = -tf.reduce_sum(
            self.f(x_r) * tf.math.log(self.f(x_r) + 1e-20), axis=1
        )
        # labels=y_target, logits=self.f(x_r)

        return tf.reduce_mean(kl_loss + self.entropy_reg_coeff * entropy_loss)
